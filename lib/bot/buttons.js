...
export function getProjectButtons(project) {
  ...
  builder.addUrl('✏️ Edit', `${process.env.NEXT_PUBLIC_SITE_URL}/chat?project=${project.id}`)
...
export function getCreateProjectButtons(step, context = {}) {
  ...
  switch (step) {
    ...
    case 'backend':
      builder.addAction('✅ Yes, add backend', 'set_backend', { backend: true }, BUTTON_STYLES.PRIMARY)
      builder.addAction('❌ No backend', 'set_backend', { backend: false })
      break
    ...
    case 'complete':
      builder.addUrl('🌐 Open in WorkersCraft', 
        `${process.env.NEXT_PUBLIC_SITE_URL}/chat?project=${context.projectId}`)
      break
  }
